...
from mcp_xsearch.config import Config, load_config
...
class TestConfig:
    def test_defaults(self):
        ...
        assert cfg.server.max_result_length == 4000
...
class TestLoadConfig:
    def test_env_override(self, monkeypatch):
        monkeypatch.setenv("XSEARCH_MAX_RESULT_LENGTH", "8000")
        monkeypatch.setenv("XSEARCH_DEFAULT_BACKEND", "brave")
        cfg = load_config()
        assert cfg.server.max_result_length == 8000
        assert cfg.backends.default == "brave"

    def test_env_bool_true(self, monkeypatch):
        monkeypatch.setenv("XSEARCH_AUTO_RECOVERY_FETCH", "true")
        cfg = load_config()
        assert cfg.server.auto_recovery_fetch is True

    def test_env_bool_false(self, monkeypatch):
        monkeypatch.setenv("XSEARCH_AUTO_RECOVERY_FETCH", "false")
        cfg = load_config()
